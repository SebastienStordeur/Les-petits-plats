...
function filterWithInputValue(recipes) {
  filteredRecipes = recipes.filter((recipe) => {
    ...
    const lowerCaseDescription = recipe.description
      .toLowerCase()
      .includes(input.value.toLowerCase());
    const lowerCaseIngredients =
      recipe.ingredients.filter(({ ingredient }) =>
        ingredient.toLowerCase().includes(input.value.toLowerCase())
      ).length > 0;
...
function filterArrays(elements, index) {
  const filteredElement = elements.filter((element) => {
    return element
      .toLowerCase()
      .includes(advancedInput[index].value.toLowerCase());
  });
  lists[index].innerHTML = "";
  ...
  if (dataType === 0) {
    ...
    filteredRecipes = recipes.filter((recipe) =>
      recipe.ingredients.some(({ ingredient }) =>
        ingredient.includes(span.innerHTML)
      )
    );
  }
  if (dataType === 1)
    filteredRecipes = recipes.filter((recipe) =>
      recipe.appliance.includes(span.innerHTML)
    );
  if (dataType === 2)
    filteredRecipes = recipes.filter((recipe) =>
      recipe.ustensils.includes(span.innerHTML)
    );
}
  ...
  lists.forEach((list, index) => {
    ...
    spans.forEach((span) => {
      span.addEventListener("click", () => {
        ...
        const isFound = tags.find(
          (elementTag) => elementTag === span.innerHTML
        );
        if (!isFound) {
          ...
          tagList.forEach((tag) => {
            if (tag.classList.contains("tag0")) {
              filteredRecipes = filteredRecipes.filter((recipe) =>
                recipe.ingredients.some(({ ingredient }) =>
                  ingredient.includes(tag.innerText)
                )
              );
            }
            if (tag.classList.contains("tag1"))
              filteredRecipes = filteredRecipes.filter((recipe) =>
                recipe.appliance.includes(tag.innerText)
              );
            if (tag.classList.contains("tag2"))
              filteredRecipes = filteredRecipes.filter((recipe) =>
                recipe.ustensils.includes(tag.innerText)
              );
          });
          mainInputFiltering(filteredRecipes);
          createRecipeCard(filteredRecipes);
